...
class LeaveApplicationCompletedPage extends StatefulWidget {
  const LeaveApplicationCompletedPage({Key? key}) : super(key: key);

  @override
  _LeaveApplicationCompletedPageState createState() =>
      _LeaveApplicationCompletedPageState();
}

class _LeaveApplicationCompletedPageState
    extends State<LeaveApplicationCompletedPage>
    with AutomaticKeepAliveClientMixin<LeaveApplicationCompletedPage> {
  final ScrollController _scrollController = ScrollController();
  final LeaveCompleteBloc _bloc = GetIt.I<LeaveCompleteBloc>();

  bool isLoading = true;
  int _currentPage = 1;

  void _fetchData(int page) {
    _bloc.add(FetchLeaveCompleteEvent(page: page, perPage: 10));
  }
  ...
  Widget _buildBody() {
    return BlocBuilder<LeaveCompleteBloc, LeaveCompleteState>(
      builder: (context, state) {
        ...
        } else if (state is LeaveCompleteFailure) {
