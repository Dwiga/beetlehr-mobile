...
class CheckPlacementClockInPage extends StatefulWidget {
  final int imageId;
  final WorkingFromType workingFrom;

  const CheckPlacementClockInPage({
    Key? key,
    required this.imageId,
    required this.workingFrom,
  }) : super(key: key);
  @override
  _CheckPlacementClockInPageState createState() =>
      _CheckPlacementClockInPageState();
}

class _CheckPlacementClockInPageState extends State<CheckPlacementClockInPage> {
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      ...
      child: Scaffold(
        appBar: AppBar(title: Text(S.of(context).clock_in)),
        body: BlocListener<AcceptClockBloc, AcceptClockState>(
          listener: (context, state) {
            if (state is AcceptClockLoading) {
              ...
              Navigator.pushNamed(context, '/attendance/clock-in', arguments: {
                'data': state.data,
                'clockBody': _sendData,
              });
            } else {
  ...
  void _checkAcceptClock(OfficePlacementEntity data, LatLng latLng) {
    ...
    _acceptClockBloc.add(GetCheckAcceptClockEvent(
      _sendData!,
    ));
  }
}
