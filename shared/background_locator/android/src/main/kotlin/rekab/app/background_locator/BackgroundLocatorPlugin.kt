...
class BackgroundLocatorPlugin
    ...
    companion object {
        ...
        private val autoStart: AutoStartPermissionHelper = AutoStartPermissionHelper.getInstance()
        ...
        private fun registerLocator(
            ...
            startIsolateService(context, args)
        ...
        private fun startIsolateService(context: Context, settings: Map<*, *>) {
            ...
            intent.putExtra(
                ...
                (settings[Keys.SETTINGS_ANDROID_NOTIFICATION_CHANNEL_NAME] ?: "background_locator") as String
            )
            intent.putExtra(
    ...
    override fun onMethodCall(call: MethodCall, result: Result) {
        when (call.method) {
            ...
            Keys.METHOD_PLUGIN_UPDATE_BACKGROUND_PERMISSION -> {
                val autoStartAvailable = autoStart.isAutoStartPermissionAvailable(context!!)
                if (autoStartAvailable) {
                    val success = autoStart.getAutoStartPermission(context!!)
                    Log.d("BackgroundLocator", "UPDATE BACKGROUND PERMISSION: $success")
                    result.success(success)
                } else {
                    result.success(true)
                }
            }
            Keys.METHOD_PLUGIN_AVAILABLE_BACKGROUND_PERMISSION -> {
                val autoStartAvailable = autoStart.isAutoStartPermissionAvailable(context!!)
                Log.d("BackgroundLocator", "IS AVAILABLE BACKGROUND PERMISSION: $autoStartAvailable")

                result.success(autoStartAvailable ?: false)
            }
