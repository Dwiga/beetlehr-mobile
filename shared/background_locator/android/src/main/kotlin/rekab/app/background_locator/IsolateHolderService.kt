...
import rekab.app.background_locator.http.HttpClientService
import rekab.app.background_locator.http.RequestParams
...
import rekab.app.background_locator.service.TaskRunner
...
class IsolateHolderService : MethodChannel.MethodCallHandler, LocationUpdateListener, Service() {
    companion object {
        ...
        private val httpClient = HttpClientService.getInstance(
            OkHttpClient(),
    ...
    override fun onLocationUpdated(location: HashMap<Any, Any?>?) {
        ...
        Log.d("ONCHANGE_RESULT", location.toString())
        ...
        if (location != null) {
            try {
                val settings =
                    PreferencesManager.getSettings(context)
                val url = settings[Keys.SETTINGS_NETWORK_URL]
                val headers = settings[Keys.SETTINGS_NETWORK_HEADERS]

                Log.d("LOCATION", "URL: $url, HEADERS: $headers")
                if (url != null) {
//                sendLocationEvent(result)
                    val body = JSONObject(
                        "{\"latitude\": \"${location[Keys.ARG_LATITUDE]}\",\"longitude\": \"${location[Keys.ARG_LONGITUDE]}\"}"
                    )

                    var headersData = hashMapOf<String, Any>()

                    if (headers != null) {
                        headersData = Gson().fromJson(
                            headers as String,
                            HashMap::class.java
                        ) as HashMap<String, Any>
                    }

                    httpClient.run(RequestParams(url.toString(), body.toString(), headersData))
                }
            } catch (e: Exception) {
                Log.d("Location", "Exception Error: ${e.message}")
            }
        }
    }
